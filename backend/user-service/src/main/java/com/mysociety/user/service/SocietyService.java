...
import com.mysociety.user.repository.SocietyRepository;
...
public class SocietyService {
    ...
    public List<Society> getActiveSocieties() {
        return societyRepository.findByActive(true);
    }
    ...
    public Map<String, Object> generateScannerCode(String societyName, String generatedBy) throws Exception {
        Optional<Society> societyOpt = societyRepository.findByName(societyName);
        if (!societyOpt.isPresent()) {
            throw new Exception("Society not found: " + societyName);
        }

        Society society = societyOpt.get();
        String scannerCode = "SCN-" + Math.abs(UUID.randomUUID().toString().replace("-", "").hashCode()) + "-" + System.currentTimeMillis();
        long now = System.currentTimeMillis();

        society.setScannerCode(scannerCode);
        society.setScannerCodeUpdatedAt(now);
        society.setUpdatedAt(now);
        societyRepository.save(society);

        Map<String, Object> response = new HashMap<>();
        response.put("societyName", society.getName());
        response.put("scannerCode", scannerCode);
        response.put("scannerPayloadTemplate",
            "{\"scannerCode\":\"" + scannerCode + "\",\"visitorType\":\"GUEST\",\"serviceSubCategory\":\"\",\"visitorName\":\"\",\"visitorAadhaar\":\"\",\"visitorPhone\":\"\",\"tower\":\"\",\"flatNumber\":\"\",\"ownerName\":\"\",\"ownerPhone\":\"\",\"faceCapture\":\"\"}");
        response.put("updatedAt", now);
        response.put("generatedBy", generatedBy == null ? "UNKNOWN" : generatedBy);
        return response;
    }

    public Map<String, Object> getScannerCode(String societyName) throws Exception {
        Optional<Society> societyOpt = societyRepository.findByName(societyName);
        if (!societyOpt.isPresent()) {
            throw new Exception("Society not found: " + societyName);
        }
        ...
        Map<String, Object> response = new HashMap<>();
        response.put("societyName", society.getName());
        response.put("scannerCode", society.getScannerCode());
        response.put("updatedAt", society.getScannerCodeUpdatedAt());
        return response;
    }
